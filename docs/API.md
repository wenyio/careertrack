...
**请求体:** 同 GET 响应结构。支持局部更新；`job_intentions` 可保存多条求职意向，
简历创建或编辑导入时只会选取其中一条写入简历 `content.basic_info.job_intention`。
兼容字段 `basic_info.job_intention` 保留为默认求职意向的单条内容。`self_evaluations`
可保存多条自我评价，简历创建或编辑导入时只会选取其中一条写入简历 `content.summary`。
兼容字段 `summary` 保留为默认自我评价的单条内容。可清空的富文本字段（如 `summary`、`self_evaluations[].description`、
...
- `field` 支持 `education`、`skills`、`work_experience`、`projects`、`portfolio`、`awards`、`other_experience`、`research`、`self_evaluations`、`job_intentions`。其中 `self_evaluations` 用于从简历编辑页将当前单条自我评价新增到个人信息，或覆盖个人信息中已有的一条自我评价；`job_intentions` 用于从简历编辑页将当前单条求职意向新增到个人信息，或覆盖个人信息中已有的一条求职意向。
