...
- 安全冒烟脚本可自动启动并清理独立测试服务；最终验收通过 139 项单测、100 项 E2E、生产构建和 7 项安全冒烟
- 新增 `test:security-smoke`，覆盖 HttpOnly 会话、并发注册码、revision 冲突、公开 DTO、JSON-LD XSS、MCP 禁用用户和登出

### 已知限制

- 当前限流器为单进程内存实现；多实例生产部署必须在反向代理或 Redis 等共享存储层增加全局限流
- JWT 会话尚未引入服务端 session/version；登出会清除浏览器 Cookie，但已复制的 JWT 只能依赖到期、账号禁用或密钥轮换失效
