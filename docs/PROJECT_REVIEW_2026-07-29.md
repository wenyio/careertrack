...
优先处理入口的交互规则进一步收敛为可测试的分桶策略：待规划申请的主按钮是“安排下一步”，打开跟进记录器并默认要求选择日期；逾期和今日事项的主按钮是“处理”，避免无意识保留已经到期的旧日期；面试阶段的今日、逾期和未来事项直接进入“记录面试”；未来普通事项仍保持“记录进展 + 保持当前”。所有优先事项统一提供“详情”次按钮且不自动打开记录器，“待跟进”统计只定位到优先处理模块，不再自动展开未来和待规划事项。
