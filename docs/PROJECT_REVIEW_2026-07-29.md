...
- 简历编辑页支持单条数组模块记录与个人信息双向显式同步，可新增或覆盖指定 profile 条目，也可从 profile 条目更新当前简历记录，避免用户在两处重复编辑；
- 个人信息页将“个人简介”调整为“自我评价”并支持多条 profile 记录；简历 `summary` 保持单条，创建与导入时只取默认自我评价，并可将当前简历自我评价新增或覆盖同步回 profile，避免影响模板和历史简历；
- 基本信息工作年限支持“下拉选项 + 自定义数字输入”，Profile 页和简历编辑页共用同一归一化规则；
- 文档已同步到 v1.0.3。
...
头像和简历侧单条求职意向仍留在父表单，Profile 侧求职意向已演进为可维护多条，
简历创建、导入和预览继续消费选中的单条兼容数据。本轮同时修复隐藏字段会丢失 `avatar_left` 的问题，
为字段操作补齐键盘入口和可访问名称，并让工作年限在 Profile 页与简历编辑页
共享“下拉选项 + 自定义数字输入”的同一控件行为。
...
国际化已进入分阶段实施：第一轮完成语言状态、Ant Design locale、主导航、登录、404、简历列表、个人信息与公开简历基础文案；第二轮继续覆盖注册、OAuth 回调、游客数据迁移、简历新建/重命名、公开链接，以及账号安全中的用户名、密码和 GitHub 绑定设置；第三轮覆盖 OTP 启用/禁用/恢复码流程、认证 hooks 与 MCP Key 管理/接入指南；第四轮覆盖后台概览、用户、简历、注册码、详情页和后台只读个人信息面板；第五轮覆盖求职进展列表、优先处理、进展概览、申请表单、详情抽屉、时间线和相关操作 hooks；第六轮覆盖简历编辑器模板选择/设置、顶部工具栏、版本历史抽屉和简历/个人信息相关操作提示；第七轮覆盖富文本编辑工具栏、链接/颜色弹层和工作经历城市 placeholder。剩余重点是求职信产品范围。
...
| 国际化、英文模板、求职信 | 中 | 中高 | 配置去重、i18n | 已覆盖核心导航、认证、安全、MCP、后台、求职进展和编辑器模板/版本历史；求职信仍待产品化 |
