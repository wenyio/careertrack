...
国际化已进入分阶段实施：第一轮完成语言状态、Ant Design locale、主导航、登录、404、简历列表、个人信息与公开简历基础文案；第二轮继续覆盖注册、OAuth 回调、游客数据迁移、简历新建/重命名、公开链接，以及账号安全中的用户名、密码和 GitHub 绑定设置；第三轮覆盖 OTP 启用/禁用/恢复码流程、认证 hooks 与 MCP Key 管理/接入指南。剩余重点是后台、求职进展详情和英文模板/求职信。
