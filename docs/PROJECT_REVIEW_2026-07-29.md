...
国际化已进入分阶段实施：第一轮完成语言状态、Ant Design locale、主导航、登录、404、简历列表、个人信息与公开简历基础文案；第二轮继续覆盖注册、OAuth 回调、游客数据迁移、简历新建/重命名、公开链接，以及账号安全中的用户名、密码和 GitHub 绑定设置；第三轮覆盖 OTP 启用/禁用/恢复码流程、认证 hooks 与 MCP Key 管理/接入指南；第四轮覆盖后台概览、用户、简历、注册码、详情页和后台只读个人信息面板；第五轮覆盖求职进展列表、优先处理、进展概览、申请表单、详情抽屉、时间线和相关操作 hooks。剩余重点是英文模板/求职信。
...
| 国际化、英文模板、求职信 | 中 | 中高 | 配置去重、i18n | 已覆盖核心导航、认证、安全、MCP、后台和求职进展；后续继续扩展英文模板与求职信 |
