...
后续已将列表页重组为实际工作台：行动中心、阶段视图和管理列表共享状态配置与 revision 保护。
...
数据库会话时区决定“今天”和“逾期”的归类。阶段页明确标注卡片仅为当前页的最近更新，避免把分页数据伪装成完整看板。

后续产品设计收敛为当前项目的 Ant Design 工作台风格：页面上方固定呈现优先处理和进展概览，底部再切换最近动态、
全部申请和阶段视图。创建、编辑和查看详情统一采用右侧“申请工作台”抽屉，避免新建弹窗与详情抽屉混用造成空间模型割裂。
